...
class FactionManagement(commands.Cog):
    ...
    async def create_faction(self, ctx, faction_name: str):
        ...
        if current_faction:
            await ctx.send(
                f"You are already a member of `{current_faction}`. Leave your current faction first."
            )
            return

        if await is_faction_name_taken(faction_name):
            await ctx.send(
                "That faction name is already taken. Please choose another name."
            )
            return
        ...
        await ctx.send(
            f"Faction `{faction_name}` has been created! You are now the leader."
        )

    @commands.hybrid_command(
        name="invite", description="Invite a user to your faction."
    )
    async def invite(self, ctx, target: discord.Member):
        ...
        if not inviter_faction:
            await ctx.send(
                "You are not part of a faction. Create one with `/create_faction`."
            )
            return
        ...
        if target_faction:
            await ctx.send(
                f"{target.display_name} is already a member of `{target_faction}`."
            )
            return
        ...
        try:
            ...
            await ctx.send(
                f"Could not DM {target.display_name}. They may have DMs disabled."
            )
            return
        ...
        try:
            ...
            await ctx.send(
                f"{target.display_name} did not respond to the invite in time."
            )
            return
        ...
        if response.content.lower() == "yes":
            ...
            await ctx.send(
                f"{target.display_name} has joined your faction `{inviter_faction}`!"
            )
    ...
    @commands.hybrid_command(
        name="leave_faction", description="Leave your current faction."
    )
    async def leave_faction(self, ctx):
        ...
        if await is_leader(user_id):
            await ctx.send(
                "You cannot leave the faction as the leader. Disband the faction instead."
            )
            return
    ...
    @commands.hybrid_command(
        name="disband_faction", description="Disband your faction."
    )
    ...
    @commands.hybrid_command(
        name="faction_members", description="List members of your faction."
    )
